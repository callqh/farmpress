...
export const _build = async () => {
  ...
  try {
    ...
    const render = await import(path.resolve(cwd(), "./docs_build/ssr/index.js")).then(res => res.default);
    const html_template = fs.readFileSync(path.resolve(cwd(), "./index.html"), "utf-8");
    console.log(routers, '===')
    ...
    for (const item of routers) {
      ...
      console.log(html, '==')
      let htmlContent = html_template.replace('<!--<?- DOC_CONTENT ?>-->', html);

      // 确保目录存在
      await fs.ensureDir(path.resolve(cwd(), './docs_build', './' + item.path));
      await fs.writeFile(path.resolve(cwd(), './docs_build', './' + item.path, 'index.html'), htmlContent);
    }
  } catch (error) {
